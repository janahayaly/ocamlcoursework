...
type gctx = {
  ...
  y: int;
  color: color;  
  (*Adding thickness to the gctx type*)
  thickness: int; (** the current pen color *)
  }
...
let top_level : gctx =
  { x = 0;
    y = 0;
    color = black;
    (*setting the initial value of thickness to 1*)
    thickness = 1;
  }
...
(* a function which produces a new Gctx.t with a new thickness *)
...
(* changing the graphics context to also change line width *)
let set_graphics_state (gc: gctx) : unit =
  let c = gc.color in
  Graphics.set_color (Graphics.rgb c.r c.g c.b);
  Graphics.set_line_width (gc.thickness)
...
let draw_point (gc : gctx) (p : position) : unit = 
set_graphics_state gc;
  let (x, y) = ocaml_coords gc p in  
  Graphics.plot x y

let draw_points (gc : gctx) (l : position list) : unit =
set_graphics_state gc;
  List.iter (fun p -> draw_point gc p) l
...
let draw_rect (g: gctx) (p1: position) ((w, h): dimension) : unit =
  set_graphics_state g;
  (* adjusting coordinates to ocaml standards*)
