...
module LinkedQueue : QueueInterface.QUEUE = struct
  ...
  let from_list (l: 'a list) : 'a queue =
    let q = create () in
    let rec listrec (l: 'a list) : 'a queue =
      begin match l with
        | [] -> q.head <- None; q.tail <- None; q
        | [hd] -> enq hd q; q
        | hd :: tl -> enq hd q; listrec(tl)
      end
    in
    let eq = listrec l in
    eq
  ...
  let contains (elt: 'a) (q: 'a queue) : bool =
    ...
    let rec loop (no:'a qnode option) (cont: bool) : bool =
      begin match no with
        | None -> cont
        | Some {v; next} -> loop {v; next}.next (v == elt || cont)
      end
    in
    loop q.head false
  ...
  let truncate (elt: 'a) (q: 'a queue) : unit =
    ...
    let rec loop (curr:'a qnode option): unit =
      begin match curr with
        | None -> ()
        | Some n -> if n.v == elt then (q.tail <- curr; n.next <- None)
        else loop n.next
      end
      in
    loop q.head
      ...
      begin match curr with
        ...
        begin match prev with
          | None -> 
            ...
            | Some _ -> q.head <- n.next
            end
          | Some x -> 
            begin match n.next with
            | None -> x.next <- None; q.tail <- Some x
            | Some _ -> x.next <- n.next; n.next <- None; loop x.next prev
            end
          end)
